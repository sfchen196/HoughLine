...
int main(int argc, char *argv[])
{
    ...
    if (flag)
    {
        ...
        features::HoughLines(edge_image, lines_from_edges, max_line_from_edges, config["HoughLine_params"]["d_rho"].as<double>(),
                             config["HoughLine_params"]["d_rho"].as<double>(), config["HoughLine_params"]["threshold"].as<int>(),
...
*/
    std::vector<cv::Point> pts_from_edges = features::reverse_roi(roi_topleft, features::polarLine2cartPoints(max_line[0], max_line[1], distance_factor_btw_pts));
